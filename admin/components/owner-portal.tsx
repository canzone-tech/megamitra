...
export function OwnerPortal({ section }: { section: Section }) {
  ...
  function winnerAction(selected: Row, winner: Row) {
    ...
    if(drawStatus==='PUBLISHED'){
      ...
      if(claimStatus==='PENDING') return <button type="button" className={styles.button} disabled={busy} onClick={()=>claimWinner(winnerId)}>START CLAIM</button>;
      if(claimStatus==='CLAIMED') return <form method="post" onSubmit={(event)=>fulfillWinner(event,winnerId)}><div className={styles.buttonLine}><input name="externalReference" className={styles.input} required placeholder="Delivery / tracking ref" /><button className={classNames(styles.button,styles.green)} disabled={busy}>MARK FULFILLED</button></div></form>;
    ...
    if(text(winner.verificationStatus)!=='VERIFIED') return <button type="button" className={classNames(styles.button,styles.outline)} onClick={()=>verifyWinner(winnerId)} disabled={busy}>VERIFY PASS</button>;
    return 'Verified';
  }

  function renderPrizes() {
    const seasons = (Array.isArray(data) ? data : []) as Row[];
    return <><Hero title="Prize Catalogue" subtitle="Manage prize categories, values, quantities, winner limits and monthly allocation." /><div className={styles.card}><SectionHead icon="▣" title="Prize Schedule" /><Field label="Season"><select className={styles.select} value={selectedSeasonId} onChange={(event)=>void changePrizeSeason(event.target.value)}><option value="">Select season</option>{seasons.map((row)=><option key={text(row.id)} value={text(row.id)}>{text(row.name)} • {text(row.status)}</option>)}</select></Field></div>{selectedSeasonId ? <div className={styles.card}><SectionHead icon="◆" title="Monthwise Prizes" action={<button type="button" className={styles.button} onClick={addPrize}>+ ADD PRIZE</button>} />{prizeDraft.length ? <div className={styles.months}>{prizeDraft.map((prize,index)=><div className={styles.month} key={`${prize.prizeCode}-${index}`}><span className={styles.monthNo}>{prize.monthNumber}</span><h3>MONTH {prize.monthNumber}</h3><div className={styles.prizeVisual}>◆</div><Field label="Month"><input className={styles.input} type="number" min="1" max="60" value={prize.monthNumber} onChange={(event)=>updatePrize(index,'monthNumber',Number(event.target.value)||1)} /></Field><Field label="Prize Name"><input className={styles.input} value={prize.name} onChange={(event)=>updatePrize(index,'name',event.target.value)} /></Field><Field label="Category"><input className={styles.input} value={prize.category} onChange={(event)=>updatePrize(index,'category',event.target.value)} /></Field><Field label="Winner Count"><input className={styles.input} type="number" min="1" value={prize.winnerCount} onChange={(event)=>updatePrize(index,'winnerCount',Number(event.target.value)||1)} /></Field><Field label="Approx. Value (optional)"><input className={styles.input} inputMode="decimal" value={prize.nominalValue??''} onChange={(event)=>updatePrize(index,'nominalValue',event.target.value)} /></Field><div className={styles.buttonLine}><button type="button" className={classNames(styles.button,styles.red)} onClick={()=>removePrize(index)}>REMOVE</button></div></div>)}</div>:<Empty>No prizes yet. Use “+ Add Prize” to build the monthwise schedule.</Empty>}<div className={styles.buttonLine}><button type="button" className={styles.button} disabled={busy||!prizeDraft.length} onClick={savePrizes}>SAVE PRIZE SCHEDULE</button></div></div>:null}</>;
  }
  ...
  async function claimWinner(winnerId:string){if(!selectedDrawId)return;const result=await act(()=>apiJson<Row>(`${API}/draws/${encodeURIComponent(selectedDrawId)}/winners/${encodeURIComponent(winnerId)}/claim`,{method:'POST',body:'{}'}),'Prize claim started',false);if(result)setData(result);}
  async function fulfillWinner(event:FormEvent<HTMLFormElement>,winnerId:string){event.preventDefault();if(!selectedDrawId)return;const form=new FormData(event.currentTarget);const result=await act(()=>apiJson<Row>(`${API}/draws/${encodeURIComponent(selectedDrawId)}/winners/${encodeURIComponent(winnerId)}/fulfill`,{method:'POST',body:JSON.stringify({externalReference:formString(form,'externalReference')})}),'Prize marked fulfilled',false);if(result)setData(result);}
  ...
  function addPrize(){setPrizeDraft((current)=>{const nextMonth=current.length?Math.max(...current.map((item)=>item.monthNumber)):1;const month=Math.min(60,nextMonth);return[...current,{monthNumber:month,prizeCode:`PRIZE_${Date.now().toString(36).toUpperCase()}`,category:'Prize',name:'New Prize',winnerCount:1}];});}
  function removePrize(index:number){setPrizeDraft((current)=>current.filter((_,i)=>i!==index));}
